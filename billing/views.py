...
from .services import RobokassaService, PallyService, PaymentService, logger
    ...
    def post(self, request):
        ...
        try:
            ...
            if not items_data:
                ...
                return Response(
                    {"error": "Не все обязательные параметры указаны"}, status=400
                )
            ...
            for item in items_data:
                if item["product_type"] == "subscription_service":
                    sub_items = SubscriptionPurchaseService.prepare_subscription_purchase_items(
                        item
                    )
            ...
            if not items_for_payment:
                return Response(
                    {"error": "Нет корректных товаров для оплаты"}, status=400
                )

            payment = PallyService.create_payment(
                username=username, email=email, items=items_for_payment
            )
            logger.info(
                f"Created payment: ID={payment.id}, invoice={payment.invoice_id}, amount={payment.amount}"
            )

            payment_url = PallyService.get_payment_url(payment, items_for_payment)
            logger.info(f"Generated payment URL, length: {payment_url}")
...
class CompareLoginsTest(APIView):
    def get(self, request):
        ...
        try:
            ...
            signature1 = (
                hashlib.md5(signature_value1.encode("utf-8")).hexdigest().lower()
            )

            login2 = "Psgamezz"
            signature_value2 = f"{login2}:{out_sum}:{inv_id}:{test_password}"
            signature2 = (
                hashlib.md5(signature_value2.encode("utf-8")).hexdigest().lower()
            )
...
class TestPaymentWithBoth(APIView):
    def get(self, request):
        ...
        try:
            ...
            url1 = RobokassaService.get_payment_url(
                test_payment, merchant_login="PSWIN.RU", items_data=[]
            )
            url2 = RobokassaService.get_payment_url(
                test_payment, merchant_login="PSWIN.RU", items_data=[]
            )
...
class PaymentResult(APIView):
    def post(self, request):
        logger.info(f"=== ПОЛУЧЕНО УВЕДОМЛЕНИЕ ОТ PAYPALYCH ===")
        ...
        success, message = PallyService.process_payment(data)
...
class PaymentSuccess(APIView):
    def post(self, request):
        logger.info(f"=== ПОЛУЧЕНО SUCCESS===")
        ...
        if not payment:
            ...
            return Response({"message": f"Payment with id - {inv_id} not found"}, status=404)
        logger.info(f"https://psgamezz.ru/?pid={payment.id}")
        logger.info(f"Request data: {data}")
        logger.info(f"Payment ID: {payment.id}")

        logger.info(f"=== КОНЕЦ ОБРАБОТКИ УВЕДОМЛЕНИЯ ===")

        return redirect(f"https://psgamezz.ru/?pid={payment.id}")


@method_decorator(csrf_exempt, name="dispatch")
class PaymentFailed(APIView):
    def post(self, request):
        logger.info(f"=== ПОЛУЧЕНО Failed===")
        ...
        return redirect("https://psgamezz.ru/?payment=failed")
